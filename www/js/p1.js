...
function trianguloAtras(onclickFn){
	var bcr=getBcr();
	var y=bcr.height/5;
	var x=bcr.width/1.1;
	var size=Math.min(bcr.width, bcr.height)/6;
	var points=x+","+y+" "+x+","+(y+size)+" "+(x-(size/2))+","+(y+(size/2));
	return html('polygon',{onclick:onclickFn, points:points, style:"fill:DodgerBlue ;stroke:purple;stroke-width:1"}," ");
}
var colorOverMag={gray:{style:backColor('gray')+"color: white"},red:{style:backColor('red')+"color: white"}, orange:{style:backColor('orange')+"color: black"}, yellow:{style:backColor('yellow')+"color: black"}, green:{style:backColor('green')+"color: white"}};
function getOverColour(mag, value){
	...
	else if(overflow>=1) return colorOverMag.red;
	else if(overflow>=.75) return colorOverMag.orange;
	else if(overflow>=.5)  return colorOverMag.yellow;
	else return colorOverMag.green;
}
...
function getStyle(zone){
	var ls=data.filterByZones([zone.name], data.lines);
...
function zoneClicked(zoneAbrv){
	for(var idx in zones)
		if(zones[idx].abrv==zoneAbrv){
			myPieP5.setZoneClicked(zones[idx].abrv);
			myBarP5.setZoneClicked(zones[idx].abrv);
		}
	p1Clicked();
}
...
function infoMagnitud(mag, lines){
	...
	if(trs.length>0){
		...
		var tr1=tr( td({rowspan:2}, "Colores")+td({width:'10px'},' ')+
				td(colorOverMag.green,'Verde')+td('<')+td(li.limite*.5)+td({width:'20px'},' ')+
				td(colorOverMag.orange,'Naranja')+td('>=')+td(li.limite*.75)
				);
		var tr2=tr( td()+
				td(colorOverMag.yellow,'Amarillo')+td('>=')+td(li.limite*.5)+td('')+
				td(colorOverMag.red,'Rojo')+td('>=')+td(li.limite)
				);
	...
	if(lines.length>0){
		...
		for(klo in linesOrder){
			...
			for(var i in linesO){
				var lin = linesO[i];
				var h=data.lastIdxWithValue(lin.values)+1;
				trs2+=tr(
	...
	return	div({id:mag.abrv+"1","class":"general",style:estilo.style,onclick:"clickMag('"+mag.abrv+"')"},mag.name+" ("+mag.abrv+")")+
...
function helpClicked(elem){
	var content=document.getElementById(elem.id+"content");
	var tengo=content.style.display;
	document.getElementById("pinicialcontent").style.display="none";
	document.getElementById("pgraficoscontent").style.display="none";
	document.getElementById("pcontaminantescontent").style.display="none";
	content.style.display=(tengo=="none"?"block":"none");
}
